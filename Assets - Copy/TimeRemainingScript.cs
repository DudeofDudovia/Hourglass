    ...
    void Update()
    {
        if (!TCS.CountUp)
        {
            if (TCS.MSUsedDisplay)
            {
                ...
                {
                    //TMP.text = (((int)(TCS.RemainingTime/60f))).ToString() + " H";
            ...
            else
            {
                ...
                else
                {
                    ...
                    seconds *= 60;
                    //seconds -= 3600;
                    TMP.text = (TCS.Truncate((TCS.RemainingTime - (TCS.Truncate(((TCS.RemainingTime / 60f) % 60), 0)) * 60), 0).ToString() + "M : " + TCS.Truncate(seconds, 0) + "S");*/
                    if (TCS.RemainingTime <= -60)
                    {
                        //TMP.text = (((int)(TCS.RemainingTime/60f))).ToString() + " H";
                        float seconds = TCS.RemainingTime - TCS.Truncate((TCS.RemainingTime - (TCS.Truncate(((TCS.RemainingTime / 60f) % 60), 0)) * 60), 0);
        ...
        else
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    seconds *= 60;
                    //seconds -= 3600;
                    TMP.text = (TCS.Truncate((TCS.UsedTime - (TCS.Truncate(((TCS.UsedTime / 60f) % 60), 0)) * 60), 0).ToString() + "M : " + TCS.Truncate(seconds, 0) + "S");*/
                    if (TCS.UsedTime <= -60)
                    {
                        //TMP.text = (((int)(TCS.UsedTime/60f))).ToString() + " H";
                        float seconds = TCS.UsedTime - TCS.Truncate((TCS.UsedTime - (TCS.Truncate(((TCS.UsedTime / 60f) % 60), 0)) * 60), 0);
